...
class UserData:
    ...
    def get_formatted_username(self, username: str) -> str:
        """Форматирование имени с галочкой верификации ☑️"""
    ...
    def add_friend(self, user_id: int, friend_username: str) -> Dict:
        ...
        # Добавляем входящую заявку другу
    ...
    def accept_friend(self, user_id: int, friend_username: str) -> Dict:
        ...
        # Добавляем в друзья к отправителю
    ...
    def create_duel(self, creator_id: int, opponent_username: str, duration: int, bet: int = 0) -> Dict:
        ...
        return {
            "success": True, 
            "message": f"✅ Дуэль создана! Ожидание ответа от @{opponent_username}",
            "duel_id": duel_id
        }
    ...
    def accept_duel(self, user_id: int, duel_id: str) -> Dict:
        ...
        return {
            "success": True,
            "message": f"✅ Дуэль принята! Бой начнется через 5 секунд!",
            "duel_id": duel_id
        }
    ...
    def finish_duel(self, duel_id: str) -> Dict:
        ...
        if duel["creator_clicks"] > duel["opponent_clicks"]:
            ...
            winner_clicks = duel["creator_clicks"]
            loser_clicks = duel["opponent_clicks"]
            ...
            winner_clicks = duel["opponent_clicks"]
            loser_clicks = duel["creator_clicks"]
    ...
    def get_user_duels(self, user_id: int) -> List[Dict]:
        """Получение всех дуэлей пользователя"""
        user_duels = []
        for duel_id, duel in self.duels.items():
            if user_id == duel["creator_id"] or user_id == duel["opponent_id"]:
                duel["id"] = duel_id
...
async def check_and_send_channel_promos(application: Application):
    """Проверка и отправка промокодов в канал"""
    while True:
        try:
...
async def send_channel_promocodes(context):
    """Отправка промокодов в канал по расписанию"""
    try:
        bot = context.bot
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, username: str):
    ...
    # Проверяем наличие заявок в друзья
...
async def friend_add_username(update: Update, context: ContextTypes.DEFAULT_TYPE, username: str):
    """Обработка добавления друга по логину"""
        ...
        if friend_account and friend_account.get("telegram_id"):
            ...
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"Перейдите в раздел 👥 Друзья → 📨 Входящие заявки, чтобы принять или отклонить заявку."
                ),
                parse_mode='HTML'
            )
    
    del user_states[user_id]
    await show_main_menu(update, context, user_id, user_storage.get_account_by_telegram_id(user_id).get("username"))
...
async def duel_create(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Создание дуэли - выбор противника"""
    query = update.callback_query
    await query.answer()
...
async def duel_opponent(update: Update, context: ContextTypes.DEFAULT_TYPE, opponent_username: str):
    ...
    # Сохраняем противника в состоянии
...
async def duel_duration(update: Update, context: ContextTypes.DEFAULT_TYPE, duration: int):
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton("💰 50,000", callback_data=f"duel_bet_{opponent}_{duration}_50000")
        ],
        [InlineKeyboardButton("💰 Без ставки", callback_data=f"duel_bet_{opponent}_{duration}_0")],
...
async def duel_click(update: Update, context: ContextTypes.DEFAULT_TYPE, duel_id: str):
    ...
    if result["success"]:
        ...
        if duel["status"] == "finished":
            ...
            if duel["winner"] == duel["creator_username"]:
                creator_data = user_storage.get_user(duel["creator_id"])
                creator_data["duels_won"] = creator_data.get("duels_won", 0) + 1
                user_storage.update_user(duel["creator_id"], creator_data)
                
                opponent_data = user_storage.get_user(duel["opponent_id"])
                opponent_data["duels_lost"] = opponent_data.get("duels_lost", 0) + 1
                user_storage.update_user(duel["opponent_id"], opponent_data)
                
            elif duel["winner"] == duel["opponent_username"]:
                creator_data = user_storage.get_user(duel["creator_id"])
                creator_data["duels_lost"] = creator_data.get("duels_lost", 0) + 1
                user_storage.update_user(duel["creator_id"], creator_data)
                
                opponent_data = user_storage.get_user(duel["opponent_id"])
                opponent_data["duels_won"] = opponent_data.get("duels_won", 0) + 1
                user_storage.update_user(duel["opponent_id"], opponent_data)
            else:
                # Ничья
                creator_data = user_storage.get_user(duel["creator_id"])
            ...
            # Отправляем результат обоим участникам
            await context.bot.send_message(
                chat_id=duel["creator_id"],
                text=result_text,
                parse_mode='HTML'
            )
            await context.bot.send_message(
...
async def show_top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показ топа игроков с галочками"""
...
async def buy_nft_case(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    nft_categories = user_storage.get_all_nft_categories()
...
# ========== АДМИН ПАНЕЛЬ ==========
# (сокращено для экономии места, но функционал сохранен)
...
def main():
    ...
    try:
        ...
        print(f"🕐 Дуэли: 1, 3, 5, 10 минут")
        print(f"💰 Ставки: 0 - 50,000 кликов")
